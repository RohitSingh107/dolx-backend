...
!developmentChains.includes(network.name)
  ...
  : describe("Nft Marketplace Unit Tests", function () {
      let xchainge: Dolx, xchaingeContract: Dolx, xchaingeToken: DolxToken
      ...
      beforeEach(async () => {
        ...
        xchaingeContract = await ethers.getContract("Dolx")
        xchainge = xchaingeContract.connect(deployer)
        xchaingeToken = await ethers.getContract("DolxToken", deployer)
      ...
      describe("listItem", function () {
        ...
        it("exclusively items that haven't been listed", async function () {
          await xchainge.listItem(xchaingeToken.address, TOKEN_ID, PRICE)
          const error = `Dolx__AlreadyListed("${xchaingeToken.address}", ${TOKEN_ID})`
        ...
        it("exclusively allows owners to list", async function () {
          ...
          expect(
            xchainge.listItem(xchaingeToken.address, TOKEN_ID, PRICE)
          ).to.be.revertedWith("Dolx__NotOwner")
        })
        it("needs approvals to list item", async function () {
          await xchaingeToken.approve(ethers.constants.AddressZero, TOKEN_ID)
          expect(
            xchainge.listItem(xchaingeToken.address, TOKEN_ID, PRICE)
          ).to.be.revertedWith("Dolx__NotApprovedForMarketplace")
        })
      ...
      describe("cancelListing", function () {
        it("reverts if there is no listing", async function () {
          const error = `Dolx__NotListed("${xchaingeToken.address}", ${TOKEN_ID})`
          expect(
      ...
      describe("buyItem", function () {
        it("reverts if the item isnt listed", async function () {
          ...
          ).to.be.revertedWith("Dolx__NotListed")
        })
        it("reverts if the price isnt met", async function () {
          await xchainge.listItem(xchaingeToken.address, TOKEN_ID, PRICE)
          expect(
            xchainge.buyItem(xchaingeToken.address, TOKEN_ID)
          ).to.be.revertedWith("Dolx__PriceNotMet")
        })
